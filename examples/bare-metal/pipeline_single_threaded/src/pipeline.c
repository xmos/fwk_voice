        ...
        int32_t (*output_data)[AP_FRAME_ADVANCE])
{
    ...
    md.vnr_flag = float_s32_gt(state->vnr_pred_state.output_vnr_pred, agc_vnr_threshold);

    ic_adapt(&state->ic_state, state->vnr_pred_state.input_vnr_pred, ic_output[0]);
