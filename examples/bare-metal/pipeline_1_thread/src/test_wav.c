...
extern void pipeline_init(pipeline_state_t *state, aec_conf_t *aec_conf);
...
void stage_a_wrapper(const char *input_file_name, const char* output_file_name)
{
    ...
    aec_conf_t aec_conf;
    aec_conf.num_y_channels = AEC_MAX_Y_CHANNELS;
    aec_conf.num_x_channels = AEC_MAX_X_CHANNELS;
    aec_conf.num_main_filt_phases = AEC_MAIN_FILTER_PHASES;
    aec_conf.num_shadow_filt_phases = AEC_SHADOW_FILTER_PHASES;
    
    // Initialise pipeline_state
    pipeline_state_t DWORD_ALIGNED pipeline_state;
    pipeline_init(&pipeline_state, &aec_conf);
