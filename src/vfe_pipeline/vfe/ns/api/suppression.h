...
#include "sup_echo_state.h"
#include "suppression_state.h"

void sup_test_task(chanend c_input, chanend c_output, NULLABLE_RESOURCE(chanend, c_control));
void sup_dump_parameters(REFERENCE_PARAM(suppression_state_t, state));
 ...
 */
void sup_set_noise_reset_period_ms(REFERENCE_PARAM(suppression_state_t, state), uint32_t ms);
 ...
 */
void sup_set_noise_alpha_d(REFERENCE_PARAM(suppression_state_t, state), uint32_t alpha_d);
 ...
 */
void sup_set_noise_alpha_s(REFERENCE_PARAM(suppression_state_t, state), uint32_t alpha_s);
 ...
 */
void sup_set_noise_alpha_p(REFERENCE_PARAM(suppression_state_t, state), uint32_t alpha_p);
 ...
 */
void sup_set_noise_delta(REFERENCE_PARAM(suppression_state_t, state), q8_24 delta);
 ...
 */
void sup_set_noise_floor(REFERENCE_PARAM(suppression_state_t, state), uint32_t noise_floor);
 ...
 */
void sup_reset_noise_suppression(REFERENCE_PARAM(suppression_state_t, sup));
 ...
 */
void sup_set_noise_suppression_enable(REFERENCE_PARAM(suppression_state_t, sup), int enable);
 ...
 */
void sup_init_state(REFERENCE_PARAM(suppression_state_t, state));
 ...
 * \param[in,out] sup      Suppressor state, initialised
 */
void sup_reset_echo_suppression(REFERENCE_PARAM(suppression_state_t, sup));

/** Function that suppresses residual echoes and noise in a frame
 *
 * 
 */
void sup_process_frame(vtb_ch_pair_t input[SUP_INPUT_CHANNEL_PAIRS][SUP_PROC_FRAME_LENGTH],
                      REFERENCE_PARAM(suppression_state_t, state),
                      vtb_ch_pair_t output[SUP_OUTPUT_CHANNEL_PAIRS][SUP_FRAME_ADVANCE]);
 ...
 * \param[in] enable       1 if suppresor should be enabled, 0 otherwise
 */
void sup_set_echo_suppression_enable(REFERENCE_PARAM(suppression_state_t, sup), int enable);
 ...
 */
void sup_set_echo_suppression_db(REFERENCE_PARAM(suppression_state_t, sup), int db);
 ...
 */
void sup_set_echo_suppression_mu(REFERENCE_PARAM(suppression_state_t, sup), int mu_24_8);

#endif /* SUPPRESSION_H_ */
