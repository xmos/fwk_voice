...
if [ -e fat.fs ] ; then
    ...
    if [ -e fat_mnt ] ; then
        ...
        mkdir -p fat_mnt/ww
        cp "$WW_PATH/models/common/WR_250k.en-US.alexa.bin" fat_mnt/ww/250kenUS.bin
        cp "$WW_PATH/models/common/WS_50k.en-US.alexa.bin" fat_mnt/ww/50kenUS.bin
        ...
        $FATFS_MKIMAGE_PATH/fatfs_mkimage --input=fat_mnt/ww --output=fat.fs
    fi
fi
