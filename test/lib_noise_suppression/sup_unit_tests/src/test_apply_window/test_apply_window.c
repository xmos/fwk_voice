...
int32_t half_hanning[SUPPRESSION_WINDOW_LENGTH / 2] = {
...
TEST_GROUP_RUNNER(sup_apply_window){
    RUN_TEST_CASE(sup_apply_window, case0);
}

TEST_GROUP(sup_apply_window);
TEST_SETUP(sup_apply_window) { fflush(stdout); }
TEST_TEAR_DOWN(sup_apply_window) {}

TEST(sup_apply_window, case0){
    unsigned seed = SEED_FROM_FUNC_NAME();
    int32_t second_half[SUPPRESSION_WINDOW_LENGTH /2];

    int32_t frame_int[SUP_PROC_FRAME_LENGTH];
    float_s32_t frame_fl;

    bfp_s32_t window;
    bfp_s32_t rev_window;

    double expected[SUP_PROC_FRAME_LENGTH];
    double actual;

    float_s32_t t;


    for(int v = 0; v < (SUPPRESSION_WINDOW_LENGTH / 2); v++){
        second_half[v] = half_hanning[(SUPPRESSION_WINDOW_LENGTH / 2) - v];
    }
    bfp_s32_init(&window, half_hanning, EXP, SUPPRESSION_WINDOW_LENGTH / 2, 0);
    bfp_s32_init(&rev_window, second_half, EXP, SUPPRESSION_WINDOW_LENGTH / 2, 0);
    ...
    for(int i = 0; i < 100; i++){
        ...
        for(int v = 0; v < (SUPPRESSION_WINDOW_LENGTH / 2); v++){
            ...
            t.exp = EXP;
            expected[v + (SUPPRESSION_WINDOW_LENGTH / 2)] *= float_s32_to_double(t);
        }

        for(int v = 0; v < (SUP_PROC_FRAME_LENGTH - SUPPRESSION_WINDOW_LENGTH); v++){
            expected[v + SUPPRESSION_WINDOW_LENGTH] = 0;
        }

        bfp_s32_t tmp;
        bfp_s32_init(&tmp, frame_int, EXP, SUP_PROC_FRAME_LENGTH, 0);
        sup_apply_window(&tmp, &window, &rev_window, SUP_PROC_FRAME_LENGTH, SUPPRESSION_WINDOW_LENGTH);
