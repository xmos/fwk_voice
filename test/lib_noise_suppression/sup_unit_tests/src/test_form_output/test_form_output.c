...
TEST_GROUP_RUNNER(sup_form_output){
    RUN_TEST_CASE(sup_form_output, case0);
}

TEST_GROUP(sup_form_output);
TEST_SETUP(sup_form_output) { fflush(stdout); }
TEST_TEAR_DOWN(sup_form_output) {}

TEST(sup_form_output, case0){
    unsigned seed = SEED_FROM_FUNC_NAME();

    int32_t overlap_int[SUP_FRAME_ADVANCE];
    int32_t frame_int[SUP_PROC_FRAME_LENGTH];
    int32_t output[SUP_FRAME_ADVANCE];
    float_s32_t frame_fl;
    float_s32_t overlap_fl;
    float_s32_t t;
    double ex_overlap[SUP_FRAME_ADVANCE];
    double ex_output[SUP_FRAME_ADVANCE];
    double frame_db[SUP_PROC_FRAME_LENGTH];
    double act_output;
    double act_overlap;
    ...
    for(int i = 0; i < 100; i++){
        ...
        bfp_s32_init(&overlap, overlap_int, EXP, SUP_FRAME_ADVANCE, 0);
        bfp_s32_init(&frame, frame_int, EXP, SUP_PROC_FRAME_LENGTH, 0);
        ...
        for(int j = 0; j < 5; j++){
            ...
            sup_form_output(output, &frame, &overlap);

            for(int v = 0; v <SUP_FRAME_ADVANCE; v++){
                ex_output[v] = frame_db[v] + ex_overlap[v];
                ex_overlap[v] = frame_db[v + SUP_FRAME_ADVANCE];
