...
sup_src_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), *"../test_wav_suppression/src".split("/"))
thread_speed_mhz = (600 / 5)

in_file_name = "input.wav"
out_file_name = "output.wav"
def run_sup_xe(sup_xe, audio_in, audio_out, profile_dump_file=None):
    ...
    with open(profile_dump_file, 'w') as fp:
        for line in xcore_stdo:
            fp.write(f"{line}\n")
    parse_profile_log(xcore_stdo, worst_case_file=f"sup_prof.log")
...
def parse_profile_log(prof_stdo, profile_file="parsed_profile.log", worst_case_file="worst_case.log", mapping_file="profile_index_to_tag_mapping.log"):
    ...
    sup_files = glob.glob(f'{sup_src_folder}/**/*.xc', recursive=True)
    sup_files = sup_files + glob.glob(f'{sup_src_folder}/**/*.c', recursive=True)
...
def test_profile(setup):
    sup_xe = setup
    run_sup_xe(sup_xe, "input.wav", "output.wav", "profile.log")
