...
from characterise_xc_py import generate_test_audio, process_py, process_xc,\
                                get_attenuation, rt60_type


def get_polar_response(test_id, angle_roi, step_size, noise_band, noise_db,
                        rt60, x_channel_delay, run_xc=False):
    audio_dir = test_id
    ...
    for angle in angles:
        ...
        output_file_xc = "output_{}_xc.wav".format(input_file)
        ...
        if run_xc:
            ...
            results_xc.append(attenuation_xc[-2])

    return angles, [results_py, results_xc]
...
def polar_plot(filename, description, angles, results):
    ...
    if results[1]:
        ax.plot(r, results[1], label='XC Attenuation (dB)')
    ax.grid(True)
...
def parse_arguments():
    ...
    parser.add_argument("-xc", action='store_true', help="Run XC model")
