...
def convert_input_wav(input_file, output_file):
    ...
    if chans == 6:
        ...
        # and last 2 channels are far-end audio
        subprocess.run(f"sox {input_file} -r 16000 -b 32 {output_file} remix 2 1 3 4 {extra_args}".split())
    else:
...
def pytest_sessionstart(session):
    ...
    hydra_audio_path = os.path.join(hydra_audio_base_dir, "xcore-voice_no_processing_ffd_test_suite")
