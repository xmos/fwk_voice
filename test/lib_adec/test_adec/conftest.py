...
def add_test(test_info, test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, limit_test_length_s, gt_changes, \
             allowable_false_negatives, allowable_false_positives, run_model=False, run_xcore=False, volume_changes=False):
  ...
  for pipeline_config in pipeline_configs:
    ...
    test_conf_xcore = test_conf.copy()
    if run_model:
      test_conf_model['run_xcore'] = False 
      test_list.append(test_conf_model)
    if run_xcore:
      test_conf_xcore['run_xcore'] = run_xcore 
      test_list.append(test_conf_xcore)
...
def pytest_generate_tests(metafunc):
  ...
  test_list = []
  run_xcore =  True #"HIL" #Launch the xcore version of the test
  # run_xcore =  False  #"HIL" #Launch the xcore version of the test
  ...
  for test_wav in fixed_test_files:
    ...
    add_test("stock_"+test_wav, test_list, path_to_regression_files, input_audio_files, None, None, 0, 0, allowable_false_positives, run_model=False, run_xcore=run_xcore)
  ...
  add_test("test_headrooms_gt", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, limit_test_length_s, gt_changes, 0, 0, run_model=False, run_xcore=run_xcore)
  ...
  add_test("phase_energy_spreading_1", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, limit_test_length_s, len(far_end_delay_changes), 0, 1, run_model=False, run_xcore=run_xcore)

  far_end_delay_changes = scale_delay_changes([(20, 0.0925), (40, -0.10), (60, 0.090), (80, -0.10), (100, 0.0875)])
  input_audio_files = ["e3-whats-cloud-gaming-bbc-news_squashed.wav", "busy_bar_5min_48k.wav"]
  add_test("phase_energy_spreading_2", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, limit_test_length_s, len(far_end_delay_changes), 0, 2, run_model=False, run_xcore=run_xcore)
  ...
  add_test("volume_changes", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, limit_test_length_s, len(far_end_delay_changes), 0, 4, run_model=False, run_xcore=run_xcore, volume_changes=volume_changes)

  #AEC normal mode delay change detect
  far_end_delay_changes = scale_delay_changes([(10, -0.03), (20, -0.06), (30, -0.09), (40, -0.12), (50, -0.15)])
  input_audio_files = ["e3-whats-cloud-gaming-bbc-news_squashed.wav", "busy_bar_5min_48k.wav"]
  add_test("aec_delay_change", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 60, len(far_end_delay_changes), 1, 0, run_xcore=run_xcore)

  #Small +ve change (should ignore)
  far_end_delay_changes = scale_delay_changes([(10, -0.0075)])
  input_audio_files = ["e3-whats-cloud-gaming-bbc-news.wav", "busy_bar_5min_48k.wav"]
  add_test("small_mic_increase", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 20, len(far_end_delay_changes), 0, 0, run_xcore=run_xcore)

  #Small -ve change (should change)
  input_audio_files = ['happy.wav', 'busy_bar_5min_48k.wav']
  far_end_delay_changes = scale_delay_changes([(10, 0.0075)])
  add_test("small_ref_increase", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 20, len(far_end_delay_changes), 0, 0, run_xcore=run_xcore)

  #Big step change up to limits
  far_end_delay_changes = scale_delay_changes ([(10, 0.15), (20, -0.15)])
  add_test("limits", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 30, len(far_end_delay_changes), 1, 0, run_xcore=run_xcore)
  ...
  add_test("delay_at_start", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 20, len(far_end_delay_changes), 0, 0, run_xcore=run_xcore)
  ...
  add_test("rapid_changes", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 42, len(far_end_delay_changes), 2, 2, run_model=False, run_xcore=run_xcore)
  ...
  add_test("tricky", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 35, len(far_end_delay_changes), 1, 1, run_model=False, run_xcore=run_xcore)
  ...
  add_test("sudden_noise_stuck_adec", test_list, path_to_regression_files, input_audio_files, far_end_delay_changes, 28, len(far_end_delay_changes), 0, 0, run_xcore=run_xcore)
  ...
  if do_random_tests:
    ...
    input_audio_files = ['happy.wav', 'busy_bar_5min_48k.wav']
    add_test("random_gt", test_list, path_to_regression_file, input_audio_files, far_end_delay_changes, limit_test_length_s, gt_changes, 1, 0, run_model=False, run_xcore=run_xcore)
  ...
  test_name_list = [test["info"]+("_xcore" if test["run_xcore"] else "_model") for test in test_list]
