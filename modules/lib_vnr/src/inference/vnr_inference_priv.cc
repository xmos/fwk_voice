...
{
    quant_spec->input_scale_inv = double_to_float_s32(VNR_INPUT_SCALE_INV); //from interpreter_tflite.get_input_details()[0] call in python 
    quant_spec->input_zero_point = double_to_float_s32(VNR_INPUT_ZERO_POINT);

    quant_spec->output_scale = double_to_float_s32(VNR_OUTPUT_SCALE); //from interpreter_tflite.get_output_details()[0] call in python 
    quant_spec->output_zero_point = double_to_float_s32(VNR_OUTPUT_ZERO_POINT);
}
...
void vnr_priv_feature_quantise(int8_t *quantised_patch, bfp_s32_t *normalised_patch, const vnr_model_quant_spec_t *quant_spec) {
    ...
    bfp_s32_scale(normalised_patch, normalised_patch, quant_spec->input_scale_inv);
