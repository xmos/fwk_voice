...
void ns_update_S(bfp_s32_t * abs_Y,
    ...
    bfp_s32_t tmp;
    bfp_s32_init(&tmp, &scratch, INT_EXP, state->S.length, 0);
...
void ns_update_alpha_d_tidle(suppression_state_t * state){
    bfp_s32_t tmp;
    int32_t scratch [SUP_PROC_FRAME_BINS] = {0};
    bfp_s32_init(&tmp, &scratch, INT_EXP, SUP_PROC_FRAME_BINS, 0);

    bfp_s32_scale(&tmp, &state->p, float_s32_sub(float_to_float_s32(1.0), state->alpha_d));
    bfp_s32_add_scalar(&state->alpha_d_title, &tmp, state->alpha_d);
}
...
void ns_update_lambda_hat(bfp_s32_t * abs_Y, suppression_state_t * state){
    ...
    bfp_s32_add_scalar(&tmp2, &state->alpha_d_title, float_to_float_s32(-1.0));
    bfp_s32_mul(&tmp1, &tmp1, &tmp2);
    bfp_s32_mul(&state->lambda_hat, &state->lambda_hat, &state->alpha_d_title);
...
void ns_process_frame(bfp_s32_t * abs_Y, suppression_state_t * state){
    ...
    //adjust_exp(&state->S_min, &state->S_tmp, &state->S);
